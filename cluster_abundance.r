...
div.simpson = diversity(abund_samp, "simpson")
div.shannon = diversity(abund_samp, "shannon")

# now re-run on all the data instead
abund_all = read.csv("sero_abundance.csv", row.names=1)
abund_all = t(abund_all)
div.simpson.all = diversity(abund_all, "simpson")
div.shannon.all = diversity(abund_all, "shannon")
